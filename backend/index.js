...
const { extractCoordsFromMapLink, fetchNearbyPlaces, generateAIDescription } = require('./services/aiService');
    ...
    (async () => {
        try {
            ...
            if (link_google_maps.includes('goo.gl') || link_google_maps.includes('maps.app.goo.gl')) {
                ...
                } catch (e) { console.error('Erro ao resolver link curto:', e.message); return; }
            }
            
            const coords = extractCoordsFromMapLink(finalUrl);
            if (!coords) {
                console.log(`[IA] Não foi possível extrair coordenadas do link: ${finalUrl}`);
                return;
            }

            const places = await fetchNearbyPlaces(coords.latitude, coords.longitude);
            if (!places || places.length === 0) {
                console.log('[IA] Nenhum ponto de interesse encontrado.');
                return;
            }
            ...
            if (iaDescription) {
                ...
                console.log(`[IA] Descrição gerada e salva para o imóvel ${novoImovel.id}`);
            }
        } catch (iaError) {
            console.error('[IA] Erro no processamento:', iaError.message);
        }
    })();
});
...
app.get('/mobile/empresas/:id/imoveis', verificarToken, async (req, res) => {
    ...
    try {
        ...
        const imoveisComDados = imoveis.map(imovel => ({
            ...imovel,
            visitante_nome_completo: (imovel.visitante_nome) ? `${imovel.visitante_nome} ${imovel.visitante_sobrenome}` : null,
            fotos: fotos.rows.filter(foto => foto.imovel_id === imovel.id),
            documentos: documentos.rows.filter(doc => doc.imovel_id === imovel.id)
        }));
        res.status(200).json(imoveisComDados);
